...
tracker.bin layout (TRK2, coarse vanilla graph):
...
TRK3 (fine room graph, entrance-shuffle aware; enable via TRACKER_FORMAT = 3
once the mod evaluator supports it):
  header (38B): magic 'TRK3', u16 regionCount (fine rooms, sorted by name),
                u16 displayCount, u16 locationCount, u16 connectionCount,
                u16 nodeCount, u32 offLocations, u32 offConnections,
                u32 offNodes, u32 offRules, u32 offStrings, u32 offTattle
  Regions are the world's actual rooms; connections are the actual entrance
  graph AFTER entrance randomization, so reachability is correct under
  loading-zone shuffle. Displays are the legacy coarse areas (DISPLAYS list
  order); every room maps to one display group for map-node aggregation.
  Location (14B): u16 nameOff, u16 ruleOff (0xFFFF = TRUE), u16 region (fine),
                  u8 gswType, u16 gswId, u8 gswValue, u8 flags,
                  u8 dispGroup (display index), 2x pad
  Connection (6B): u16 src, u16 dst, u16 ruleOff
  Node (20B): char prefix[7] NUL-padded, 1x pad, u16 gateDisplay (fast travel
              unlocks when ANY room of this display group is reachable —
              same semantics as the TRK2 coarse gate region), u8 dispCount,
              u8 disp[6] (display indices), 3x pad
  Rule op 0x06 REACH_REGION takes u16 region index in TRK3 (u8 in TRK2).
"""
import json
import pkgutil
import struct
...
# Which tracker.bin format to emit. Flip to 3 once the mod-side evaluator
# understands TRK3; the __init__ clamps that disable the in-game tracker and
# fast travel under loading-zone shuffle key off this constant.
TRACKER_FORMAT = 2
...
STATELOGIC: dict = {
    ...
    "key_both": {"and": [{"has": "Red Key"}, {"has": "Blue Key"}]},
...
class TrackerCompiler:
    def __init__(self, world: "TTYDWorld"):
        ...
        # reach-region encoding; build_trk3 swaps these to the fine room graph
        self.region_idx: dict = REGION_IDX
        self.reach_region_struct = ">BB"
    ...
    def compile_expr(self, r) -> bytes:
        ...
        if "function" in r:
            ...
            if name == "PalaceAccess":
                return self.compile_expr(self.palace_expr(self.world.options.palace_stars.value))
            if name == "PalaceAccessGoal":
                return self.compile_expr(self.palace_expr(self.world.options.goal_stars.value))
            return self.compile_expr(STATELOGIC[name])
        if "can_reach" in r:
            return struct.pack(">BH", 0x05, self.loc_idx[r["can_reach"]])
        if "reach_region" in r or "can_reach_region" in r:
            region = r.get("reach_region", r.get("can_reach_region"))
            return struct.pack(self.reach_region_struct, 0x06, self.region_idx[region])
    ...
    def palace_expr(self, count: int) -> dict:
        stars_all = self.world.options.star_shuffle.value == StarShuffle.option_all
    ...
    def connections(self) -> list:
        ...
        excluded = set(self.world.excluded_regions)
        if opts.palace_skip:
            # excluded_regions holds fine-grained room names; map palace_skip
            # back onto this file's coarse region names.
            excluded.update(["Palace of Shadow", "Palace of Shadow (Post-Riddle Tower)"])
    ...
    def build_trk3(self) -> bytes:
        """Compile the fine room graph — correct under loading-zone shuffle.

        Must run after connect_entrances (generate_output does), so the
        connection list reflects the actual, possibly shuffled, entrance graph.
        """
        from .Regions import load_regions

        world = self.world
        fine_regions = sorted(region.name for region in world.multiworld.get_regions(world.player))
        self.region_idx = {name: i for i, name in enumerate(fine_regions)}
        self.reach_region_struct = ">BH"

        displays = REGIONS  # legacy coarse areas double as display groups
        display_idx = REGION_IDX
        region_display = {region["name"]: region["display"] for region in load_regions()}
        region_display["Menu"] = "Menu"
        region_display["Shadow Queen"] = "Shadow Queen"

        rules = self.location_rules()

        # Locations: logical region is the containing room; display group is
        # the coarse area (Cooking overrides for the virtual cook node).
        from .Regions import location_region_lookup
        loc_region_names = location_region_lookup()
        ...
        for loc in self.locations:
            ...
            region_name = loc_region_names.get(loc.name, "Menu")
            # excluded regions don't exist in the multiworld; park their
            # (disabled) locations on Menu
            region = self.region_idx.get(region_name, self.region_idx["Menu"])
            ...
            disp_name = "Cooking" if "cooking" in loc.tags else region_display.get(region_name, "Rogueport")
            loc_entries += struct.pack(">HHHBHBBB2x", name_off, rule_off, region,
                                       gsw_type, gsw_id, gsw_value & 0xFF, flags,
                                       display_idx[disp_name])

        # Connections: the world's actual entrance graph (post entrance rando)
        ...
        for region in world.multiworld.get_regions(world.player):
            for exit_ in region.exits:
                if exit_.connected_region is None:
                    continue
                rule_dict = world.entrance_rules.get(exit_.name)
                rule_off = 0xFFFF if rule_dict is None else self.add_rule(self.compile_expr(rule_dict))
                conn_entries += struct.pack(">HHH", self.region_idx[region.name],
                                            self.region_idx[exit_.connected_region.name], rule_off)
                connection_count += 1

        # Nodes: gate/display entries reference display groups; fast travel
        # unlocks when any room of the gate's display group is reachable.
        node_entries = bytearray()
        for prefix, gate, disp in NODES:
            disp_idx = [display_idx[name] for name in disp][:6]
            node_entries += struct.pack(">7sxHB6s3x", prefix.encode("ascii"), display_idx[gate],
                                        len(disp_idx), bytes(disp_idx).ljust(6, b"\x00"))

        tattle_table, tattle_units = self._tattle_tables()
        ...
        header = struct.pack(">4sHHHHHIIIIII", b"TRK3", len(fine_regions), len(displays),
                             len(self.locations), connection_count, len(NODES),
    ...
    def _tattle_tables(self) -> tuple:
        ...
        return tattle_table, tattle_units

    def build(self) -> bytes:
        if TRACKER_FORMAT >= 3:
            return self.build_trk3()
        world = self.world
        rules = self.location_rules()

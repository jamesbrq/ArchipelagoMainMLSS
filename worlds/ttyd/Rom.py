...
class TTYDPatchExtension(APPatchExtension):
    ...
    def patch_mod(caller: "TTYDProcedurePatch") -> None:
        ...
        caller.patcher.dol.data.write((1 if seed_options.get("loading_zone_shuffle", 0)
                                       or seed_options.get("dungeon_shuffle", 0) else 0).to_bytes(1, "big"))
        ...
        try:
            warp_data = caller.get_file("warp.txt")
        except KeyError:
            # .apttyd files from before loading-zone shuffle carry no warp table
            warp_data = b"\x00"
        caller.patcher.iso.add_new_file("files/msg/US/warp.txt", io.BytesIO(pad32(warp_data)))
...
def write_files(world: "TTYDWorld", patch: TTYDProcedurePatch) -> None:
    ...
    # Warp override table for loading-zone shuffle (mod format: null-separated
    # src_map, src_bero, dst_map, dst_bero per entry, table ends with a null)
    warp_buffer = io.BytesIO()
    for (source_map, source_bero), (target_map, target_bero) in world.warp_table.items():
        warp_buffer.write(source_map.encode("utf-8"))
        warp_buffer.write(b"\x00")
        warp_buffer.write(source_bero.encode("utf-8"))
        warp_buffer.write(b"\x00")
